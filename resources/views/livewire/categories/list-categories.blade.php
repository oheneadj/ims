<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <div class="text-sm opacity-60">Manage product categories</div>
        </div>
                ...
                <table class="table">
                    <thead class="bg-base-200">
                        <tr>
                            ...
                            <th wire:click="sortBy('slug')" class="cursor-pointer hover:bg-base-300">
                                <div class="flex items-center gap-1">
                                    Slug
                                    @if($sortBy === 'slug')
                                        <span
                    ...
                    <tbody>
                        @forelse($categories as $category)
                            <tr>
                                ...
                                <td class="font-mono text-sm opacity-70">{{ $category->slug }}</td>
                                <td class="text-sm opacity-80 truncate max-w-xs">{{ $category->description ?? '-' }}</td>
                                <td>
                                    <span class="badge badge-soft badge-neutral">{{ $category->products_count ?? 0 }}</span>
                                </td>
                                <td>
                                    <div class="flex gap-1">
                                        ...
                                        <button class="btn btn-error btn-sm" x-data
                                            x-on:click="$dispatch('open-delete-modal', { id: {{ $category->id }}, name: '{{ addslashes($category->name) }}' })">
        ...
        <div class="card bg-base-100 w-full max-w-sm shadow-2xl scale-100 transform transition-transform">
            <div class="card-body text-center">
                ...
                <div class="card-actions justify-center gap-4">
                    ...
                    <button @click="open = false; $wire.deleteCategory(id)" class="btn btn-error text-white">
                        Delete
                    </button>
                </div>
            </div>
